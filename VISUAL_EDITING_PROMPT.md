...
- Accepteer `searchParams` parameter (URLSearchParams) voor URL parameter support
- Check URL parameter `?preview=true` als fallback voor productie
- Visual editing enabled wanneer: env variable true OF URL parameter preview=true
...
- Check `PUBLIC_SANITY_VISUAL_EDITING_ENABLED` environment variable (eerste prioriteit)
- Check URL parameter `?preview=true` als fallback (voor productie)
...
- Gebruik `Astro.request.url` om query parameters door te geven: `const requestUrl = new URL(Astro.request.url)`
- Geef `searchParams: requestUrl.searchParams` door aan `loadQuery`
...
- Visual Editing kan worden geactiveerd via:
  - Environment variable: `PUBLIC_SANITY_VISUAL_EDITING_ENABLED=true` (voor development)
  - URL parameter: `?preview=true` (voor productie, zonder env variable aan te passen)
- Gebruik `export const prerender = false` voor routes die preview mode moeten ondersteunen
- Gebruik `Astro.request.url` (niet `Astro.url`) voor query parameters in SSR mode
...
- [ ] `load-query.ts`: perspective switching, resultSourceMap, stega encoding, URL parameter support
- [ ] `sanity.config.ts`: `presentationTool` plugin
- [ ] `resolve.ts`: alle document types met correcte routes
- [ ] Layout: `VisualEditing` component met enabled prop en URL parameter check
- [ ] Alle queries gebruiken `loadQuery` functie met `searchParams` parameter
- [ ] Routes hebben `export const prerender = false` voor SSR mode
- [ ] Routes gebruiken `Astro.request.url` voor query parameters
- [ ] Links behouden preview parameter wanneer actief
